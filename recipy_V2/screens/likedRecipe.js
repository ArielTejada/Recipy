...
const likeRecipePress = () => {
  if(likedRecipes.find((recipe) => recipe.id === recipeID)) {
    let newLikedRecipes = likedRecipes.filter((recipe) => recipe.id != recipeID);
      ...
      <View style={[styles.pageMargins]}>
        ...
        >
          ...
          <Pressable 
            ...
            onPress={() => {likeRecipePress()}}
          >
            <Text style={[styles.sendText, {color: likedRecipes.some(recipe => recipe.id === recipeID) ? 'white' : "black"}]}>{likedRecipes.some(recipe => recipe.id === recipeID) ? 'Liked!' : "Like This Recipe"}</Text>
          </Pressable>
        </View>
